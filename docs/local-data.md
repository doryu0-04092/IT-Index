...
設定画面の「初期データに戻す」。`backups/` へ退避 → `origin:'ai'` の語を tombstone → **語の origin を問わずすべてのノートを空にする** → `data/` を空の状態に戻す。

ノートを origin:'ai' の語だけに絞らないのは、seed 語（元から入っている語）に対しても AI・Claude Code はノートを書き足せるため（`summary` は不変だがノートは対象外）。絞ってしまうと「初期化したのにseed語への補足が残る」状態になる（実機検証で発見・2026-07-31修正）。
