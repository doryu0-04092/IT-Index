...
自動で引き継がれない(逆も同じ)。移行が必要な場合は、v1データ取り込みと同様にエクスポート/同期の手順で行う
...
- **ライセンス確認の無効化(次PRで実装予定)**: 公式ホストでは同期・運営者キーでのAI利用に有効なライセンスを要求するが
  ([architecture.md §5](./architecture.md))、セルフホストではその確認自体が不要になる設計にする。実装後は
  `LICENSE_ENABLED='0'` を設定するとライセンス要求を止められる予定(現時点ではこの環境変数はまだ存在しない)
