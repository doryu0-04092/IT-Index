...
## 5.1 ログインできなくなった時の復旧(#223)

**このアプリには「ログインできない状態からのパスワード再設定」の導線が無い。**
設定画面のパスワード変更(#220)は**ログイン中**にしか使えない。全端末でログインできなくなると、
アカウントを取り戻す手段が他に無い——ライセンスはアカウントに紐づき、同期データはサーバー上では
暗号化されていて運営者にも中身は戻せない。

その逃げ道がこの手順。**運営者(=D1に触れる人)だけが実行できる。**

### 実際に近いところまで行った事例

2026-08-22、Androidのキーボードによる入力の書き換えでログインできなくなった(#213)。
PC版で入れたため助かったが、その時「**D1をクリアするしかないのでは**」という案が出た。
**クリアするとライセンスまで消える。** そうしないための手順である。
...
# 1. 新しいパスワードのハッシュを作る(アプリと同じ形式)
node scripts/make-password-hash.mjs '新しいパスワード'
# → pbkdf2:100000:<base64 salt>:<base64 hash>

# 2. 対象アカウントを確認する(メールは小文字で保存されている。#213)
npx wrangler d1 execute it-index-v2 --remote   --command "SELECT id, email FROM accounts WHERE email = 'あなたのメール'"

# 3. password_hash を差し替える
npx wrangler d1 execute it-index-v2 --remote   --command "UPDATE accounts SET password_hash = '<1で出た値>' WHERE email = 'あなたのメール'"
```

**ローカルで試す場合は `--remote` を `--local` に置き換える。** 本番へ流す前に一度
ローカルで通しておくと確実。

### 確認できていること(実際に通した)

ローカルD1で一連を実行し、次を確かめてある。

- 新しいパスワードでログインでき、トークンが発行される
- **古いパスワードは 401 で弾かれる**
- **アカウント・ライセンス・同期データはすべて残る**(D1クリアと違い何も失われない)

### 注意

- **パスワードはシェルの履歴に残る。** 復旧後は履歴を消すか、使い捨ての値で復旧してから
  アプリの設定画面(#220)で本来のパスワードへ変更する
- `scripts/make-password-hash.mjs` は `server/src/crypto.ts` と**同じ値**
  (`ITERATIONS`/`SALT_BYTES`/`KEY_LENGTH_BITS`)を持つ。片方だけ変えると
  **復旧できないハッシュができる**ので、変える時は必ず両方を直す
- 同期データの暗号鍵(#182)は**端末のlocalStorageにあり、この手順では戻らない**。
  ログインし直しても、鍵を持つ端末が1台も無ければ同期データは読めない
  (その場合は鍵を作り直し、全端末で再pushする)

### なぜメール送信での再設定を実装しないか

送信サービスの選定・APIキー・コストが乗る。**外部の利用者がいない**ため、運営者が
D1に直接触れるこの手順で足りる。利用者が増えた場合は改めて検討する。
