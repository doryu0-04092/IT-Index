...
#### 統合ボタン領域は3状態（本人指定。2段階の実機報告を経て確定）

最初の修正は「統合済みで押し直したらAIで作り直す」で、**誤り**（採用できず、押すたびに
内容が変わって端末間の統一が終わらない）。次の修正は「採用中も『統合した内容を採用』を
出したまま」で、これも**誤り**（採用中に押しても何も変わらず、同じ内容で統合を繰り返す
動線だけが残る）。確定した形:

- **統合前（キャッシュ無し）**: 「すべての端末の内容をAIで統合」。**AIを呼ぶのはここだけ。**
  結果を全レコードの `merged` へキャッシュする
- **統合結果を採用中**: **ボタンを出さない**——「AIで統合した内容 ✓採用中」の表示で完結
- **別の内容へ選び直した（採用中が外れた）**: 「統合した内容を採用」。AIは呼ばず
  キャッシュを適用するだけ。統合結果へ戻したらボタンはまた消える

例外: **新しい端末との競合が増えたら、採用中でも「AIで統合」を出し直す**——その端末の
情報がキャッシュに入っていないため（対象全件が同じキャッシュを持つかを
`sharedMergedCache` が判定する。新レコードは `merged: null` なので自然に外れる）。
