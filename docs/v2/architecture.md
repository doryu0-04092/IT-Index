...
### 2つのキー経路(2026-08-11追加)

上流を呼ぶAPIキーには2つの経路があり、**どちらを使うかはリクエスト1件ごとに決まる**。

| 経路 | 既定 | キーの所在 | 回数上限 | 費用 |
|---|---|---|---|---|
| **サーバー側キー** | ○(何も設定しなければこちら) | サーバー(Workers Secret) | **あり**(利用者別・全体の日次上限) | 運営者負担 |
| **利用者キー(BYOK)** | — | **利用者の端末のみ**(localStorage) | **なし**(上限判定もカウントも行わない) | 利用者負担 |
...
- **利用者キーは保存も記録もしない。** D1へ書かず、ログ・エラーメッセージ・レスポンスにも載せない。
  サーバー内での寿命は1リクエスト分で、用途は上流への `Authorization` ヘッダのみ(会話内容と同じ「転送のみ」の扱い)
- 利用者キーが上流で拒否された場合(401/403)は、サーバー設定の問題(`ai_config_error`)と混同させないため
  専用コード `user_api_key_invalid` で返し、利用者を設定画面へ誘導する
- BYOKはOpenAI経路のみ。Anthropic運用時に `apiKey` が付いていたら**黙ってサーバー側キーへ落とさず400で断る**
  (落とすと「キーを付ければ上限を回避できる」抜け道になる)
- 端末側の平文localStorage保存は、v1の端末内暗号化(WebAuthn PRF / keystore)を再現しない判断
  (§1の断念の経緯)。代わりに§6第1層——**利用者自身のOpenAI支出上限(Monthly budget)設定をUIで必須案内**——で被害を有限化する
- `GET /api/ai/quota` はサーバー側キーの残量のみを表す。利用者キー利用時は意味を持たないため
  クライアントは呼ばず、「自分のAPIキーを使用中(回数上限なし)」に表示を切り替える
