...
解消をPC側へ集約する #157/#165 の設計は維持）。

#### 統合結果は「端末の行と同じ形の常設の行」（本人指定。3段階の実機報告を経て確定）

経緯: 「統合済みで押し直したらAIで作り直す」（誤り: 押すたびに内容が変わり統一が終わらない）
→「採用中も『統合した内容を採用』を出す」（誤り: 押しても何も変わらない動線が残る）
→「採用中はボタンを消し、選び直したら出す」（誤り: 採用中しか本文を出していなかったため、
別の内容を選ぶと**統合した文面ごと消え**、下部に文面の見えない採用ボタンだけが浮く）。

確定した形——**統合が終わったら「AIで統合した内容」は端末一覧の1行になり、二度と消えない**:

- 行の中身は本文＋操作で、**他の端末の行と完全に同じ**:
  採用中なら「✓採用中」バッジ、そうでなければ「こちらを採用」（AIは呼ばずキャッシュ適用）
- 下部の「すべての端末の内容をAIで統合」は**使えるキャッシュが無い時だけ**。
  **AIを呼ぶのはここだけ**で、統合後は行に役目を渡して消える
- キャッシュが無効になるのは**新しい端末との競合が増えた時**（対象全件が同じキャッシュを
  持つかを `sharedMergedCache` が判定。新レコードは `merged: null` なので自然に外れる）。
  この時は採用中でも下部の統合ボタンが復活する——ただし行は `currentChoice.merged` で
  出し続ける（いまノートに入っている内容の説明が消えるのは不当なため）

**「同じ内容での作り直し」に動線は無い。** 作り直しが必要になるのは新しい版が競合に
加わった時だけで、それは上の例外が自動的に扱う。

#### 採用ボタンと解決済みカードも常設（本人指定）

- **採用ボタンは3種とも常に表示する**（この端末 / 別の端末 / AIで統合した内容）。
  採用中はラベルを「こちらを採用中」に変える（v1と同じ形）。押しても同じ内容の再適用で無害
- **同期画面の「解決済みの競合」は同期イベントに関係なく常設する。**
  以前は直近イベントに紐づく分だけだったため、リロード時の自動pullで新しい同期イベントが
  できた瞬間、解決済みカード（統合した文面・採用ボタンごと）が消えていた（実機で報告。
  解決済みは `carryOver` の対象外）。相手が収束して `converged` で閉じた後も出し続ける——
  選び直し・統合結果の常設は、一覧に載り続けて初めて成立する。未解決の一覧は従来どおり
  直近イベント基準（`carryOver` が持ち越すため消えない）
- **保持は最新30件まで**（`RESOLVED_CONFLICTS_LIMIT`）。超えた分は古い順に
  **DBから削除**する（本人指定「削除していく方式」。表示だけ隠すのではない）。
  未解決は削除の対象にしない。削除した記録は履歴タブからも消えるが、不採用側の内容は
  `noteHistory` へ退避済みで失われない
