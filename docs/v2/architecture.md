...
| 層 | v2での内容 |
|---|---|
| 支出上限 | サーバー側キーに支出上限を設定(v1の第1層をサーバーへ移設)+利用者別回数上限(§5) |
| キーの所在 | サーバーのみ。端末・同期経路・リポジトリのいずれにも置かない |
...
## 7. サーバー基盤の選定基準(実装開始時に選定)

候補: Cloudflare Workers(+D1/Durable Objects)/ Supabase / その他の無料枠常設サービス。
**選定時点で公式ドキュメントの無料枠条件を確認して決める**(本書では確定しない)。基準:
