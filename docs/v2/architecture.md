...
#### ボタンは統合が済んだら「統合した内容を採用」に変わる（v1の形。本人指定）

最初の修正では「統合済みで押し直したらAIで作り直す」を入れたが、**これは誤りだった**
（実機で報告: 採用ボタンに変わるべきものが統合ボタンのままで採用できず、
押すたびにAIを呼んで内容が変わり、端末間の統一作業が何度でも繰り返される）。

正しい形はv1（`src/ui/shared/ConflictResolver.tsx`）と同じ:

- **キャッシュが無い** → 「すべての端末の内容をAIで統合」。AIを呼び、結果を全レコードの
  `merged` へキャッシュする
- **キャッシュがある** → 「統合した内容を採用」。**AIは二度と呼ばず**キャッシュを適用するだけ。
  元の内容へ選び直しても、このボタンからいつでも統合結果へ戻せる
- 「AIで統合」へ戻るのは、**新しい端末との競合が増えた時だけ**（その端末の情報がキャッシュに
  無いため。対象全件が同じキャッシュを持つかを `sharedMergedCache` が判定する）

**「作り直したい」に専用の動線は作らない。** 統合し直したい時は、どれかの端末の内容を
一度採用してからもう一度統合すれば良い……はキャッシュ優先のため不可で、キャッシュがある限り
同じ結果に戻る。作り直しが必要になるのは実際には「新しい版が競合に加わった時」で、
それは新しい競合レコード（`merged: null`）が増えることで自然にボタンが統合へ戻る。
