...
- 「この会話を確定する」ボタンは `commitOrchestrator.triggerCommit(sessionId)` を呼ぶ（トリガー③）。**待たない（2026-07-29変更）**——`onCommit(sessionId)` の完了を `await` せず fire-and-forget で呼び、クリックした時点で画面の操作性を利用者に返す。成否のフィードバックはこの画面のローカル状態を持たず、既存のグローバルな経路に委ねる：成功時は `App.tsx` 側の `onProposalReady` が承認画面へ遷移させ、失敗時は `commitOrchestrator` の `onError` が `App.tsx` の `globalError` に表示する（`commit()` 自体が内部で例外を握りつぶし `onError` を呼ぶだけなので `triggerCommit()` が reject することはないが、`ChatScreen` 側にも念のためログ用の `.catch()` を付けてある）
