...
- ~~設定モーダルでのDrive/手動同期~~ **端末間同期は設定モーダルではなく独立した「連携」画面として実装した**（`LinkModal.tsx`。トップナビの「設定」の左隣）。Drive同期は休眠のまま
- AI競合解決（`src/sync/resolveConflict.ts`）の承認画面。現状は連携結果に件数を表示するだけ
