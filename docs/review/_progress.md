...
### 未着手（次にここから）
- [ ] #43  ピルバッジ崩れ（`search-result-field`が長い分野名で2行折り返し崩れ）— **ブランチ未作成、作業ツリークリーン**
- [ ] #39  リロード文脈喪失（#35と方針を合わせて検討）
- [ ] #35  ブラウザバック白紙化（要方針確認: URLルーティング導入 or 軽量対処。コスト高）
- [ ] #37  非対応ブラウザバナー未実装（要仕様確認: バナー文言・表示条件・デザイン。新規実装）
- [ ] #44  性能（任意、後回し可）
