 ...
 * **同じ語が索引に2回現れることがある**——辞書の空見出しと同じで、意図した重複。「1」を
 * 「ワン」として覚えている利用者がワ行を見ても見つからなかったため。あわせて「数字」の中だけは
 * 読み順ではなく数値の昇順に並ぶ（`groupIntoBuckets`）。どちらも表示側では何もせず、
 * `src/core/kanaRow.ts` が返すバケットをそのまま描画するだけでよい。
 *
