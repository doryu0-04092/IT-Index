...
利用者がアプリ内でAIチャットを行ったものの、まだ「確定」ボタンを押していないやり取りがここに書き出されます。**このファイル自体は編集しても反映されません**（参照専用）。この内容を読み、代わりにあなたが \`data/terms.json\` への語の追加や \`data/notes/<語のid>.md\` の作成・更新を行ってください。処理が終わっても、このファイル自体は次回アプリ起動時に自動的に整理されます（手動で消す必要はありません）。
