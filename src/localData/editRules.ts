...
data/pending/*.md     … 確定前のチャットのやり取り（参照専用。あなたが編集するのは terms.json・notes/*.md 側）
backups/              … アプリが自動で作るバックアップ。編集しない
\`\`\`

## \`data/pending/<語のid>.md\` について

利用者がアプリ内でAIチャットを行ったものの、まだ「確定」ボタンを押していないやり取りがここに書き出されます。**このファイル自体は編集しても反映されません**（参照専用）。この内容を読み、代わりにあなたが \`data/terms.json\` への語の追加や \`data/notes/<語のid>.md\` の作成・更新を行ってください。

**処理が終わったら、この \`data/pending/<語のid>.md\` 自体を削除してください。** これがアプリへの「処理済み」の合図になり、アプリはこの削除を検知して、対応するチャットを確定済み（ホームの「AIによる単語更新待ち」一覧から除外）として扱います。削除しないままだと、その語は未確定のまま一覧に残り続け、後で利用者が確定ボタンを押した際にAPI側の要約処理が別途走ってしまいます。
